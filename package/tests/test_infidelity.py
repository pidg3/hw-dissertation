import numpy as np
import pytest
from exrt.infidelity import (
    calculate_infidelity,
    _order_nominal_values,
    _get_top_explanation_values,
    _calculate_perturbations,
)
...
def test_happy_path():
    result = calculate_infidelity(
        mock_explanation, mock_model, mock_instance, mock_metadata
    )
    assert result == 0.25
...
def test_explanation_not_list_like():
    ...
    assert (
        str(exception.value) == "Explanation and instance should be a list or np array"
    )
...
def test_model_has_no_predict_func():
    ...
    with pytest.raises(ValueError) as exception:
        calculate_infidelity(
            mock_explanation, MockModelNoPredict(), mock_instance, mock_metadata
        )
    ...
    with pytest.raises(ValueError) as exception:
        calculate_infidelity(
            mock_explanation, MockModelPredictError(), mock_instance, mock_metadata
        )
...
def test_calculate_perturbation_numerical():
    ...
    mock_metadata = [
        {"name": "x1", "type": "numerical", "used": True, "index": 0, "baseline": 2}
    ]
    result = _calculate_perturbations(
        instance_original, instance_perturned, mock_metadata
    )
    # difference / baseline
    assert result == [2.5]
